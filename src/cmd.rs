...
pub fn cmd_context(db: &Database, pm_dir: &Path, id: &str, include_memories: bool) {
    ...
    // Per-level artifact summary blocks. Emitted after chain bodies so the
    // composed view groups "what's in this scope" content together, with
    // the agent's referenceable assets surfaced just before memories.
    print_artifact_blocks(db, pm_dir, &chain);
    ...
    // Trailing `@`-import line so a Claude Code loader operating from the
    // leaf ticket's cwd picks up the leaf's ARTIFACTS.md as a generic
    // reference. Harmless when no `artifacts/` exists.
    println!("@artifacts/ARTIFACTS.md");
}

/// Render the per-level "## Artifacts at <LEVEL> (<id>)" blocks for each
/// chain node that has artifacts. Skips levels with no on-disk
/// `artifacts/ARTIFACTS.md` or with an empty entry list.
fn print_artifact_blocks(db: &Database, pm_dir: &Path, chain: &[crate::store::LeafId]) {
