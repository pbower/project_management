...
fn full_chain_creation_via_database_lands_on_disk_per_design_section_4() {
    ...
    db.tasks.push(fresh_task(prj, "PM tool", None, Kind::Project));
    db.save(&pm_dir).unwrap();

    let mut db = Database::load(&pm_dir);
    let prd = db.allocate_id(kind_to_prefix(Kind::Product));
    db.tasks.push(fresh_task(prd, "Core", Some(prj), Kind::Product));
    db.save(&pm_dir).unwrap();

    let mut db = Database::load(&pm_dir);
    let epc = db.allocate_id(kind_to_prefix(Kind::Epic));
    db.tasks.push(fresh_task(epc, "Checkouts", Some(prd), Kind::Epic));
    db.save(&pm_dir).unwrap();

    let mut db = Database::load(&pm_dir);
    let tsk = db.allocate_id(kind_to_prefix(Kind::Task));
    db.tasks.push(fresh_task(tsk, "Lock protocol", Some(epc), Kind::Task));
    db.save(&pm_dir).unwrap();

    let mut db = Database::load(&pm_dir);
    let sbt = db.allocate_id(kind_to_prefix(Kind::Subtask));
    db.tasks.push(fresh_task(sbt, "Stale cleanup", Some(tsk), Kind::Subtask));
    db.save(&pm_dir).unwrap();
    ...
    for dir in [&prj_dir, &prd_dir, &epc_dir, &tsk_dir, &sbt_dir] {
        ...
        assert!(dir.join(CLAUDE_MD).is_file(), "missing CLAUDE.md in {}", dir.display());
        assert!(dir.join("artifacts").is_dir(), "missing artifacts/ in {}", dir.display());
    }
...
fn parent_address_resolves_to_correct_subtree() {
    ...
    let prd = db.allocate_id(kind_to_prefix(Kind::Product));
    db.tasks.push(fresh_task(prd, "Y", Some(prj), Kind::Product));
    let epc = db.allocate_id(kind_to_prefix(Kind::Epic));
    ...
    assert!(expected.is_file(),
        "task should land at PRJ1-PRD1-EPC1-TSK1 path; got missing {}", expected.display());

    let orphan_path = pm_dir.join("tasks/TSK1/CLAUDE.md");
    assert!(!orphan_path.is_file(),
        "task should not have landed at orphan path {}", orphan_path.display());
...
fn doctor_rebuilds_state_json_from_disk_truth() {
    ...
    assert!(stdout.contains("scanned 2 tickets"), "expected scan summary; got:\n{stdout}");
...
fn pm_init_scaffolds_the_workspace() {
    ...
    assert!(output.status.success(),
        "pm init failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    for type_folder in ["projects", "products", "epics", "tasks", "subtasks", "milestones"] {
