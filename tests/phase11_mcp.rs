...
fn drive_mcp(pm_dir: &Path, lines: &[&str]) -> Vec<Value> {
    ...
    child.stdout.as_mut().expect("mcp stdout").read_to_string(&mut stdout).expect("read stdout");
    let _ = child.wait();
    stdout
        .lines()
...
fn seed_workspace(label: &str) -> PathBuf {
    ...
    assert_ok(&pm(&pm_dir, &["add", "Demo project", "--kind", "project"]), "add PRJ");
    assert_ok(&pm(&pm_dir, &["add", "Core", "--kind", "product", "--parent", "PRJ1"]), "add PRD");
    assert_ok(&pm(&pm_dir, &["add", "Checkouts", "--kind", "epic", "--parent", "PRD1"]), "add EPC");
    assert_ok(&pm(&pm_dir, &["add", "Lock protocol", "--kind", "task", "--parent", "EPC1"]), "add TSK");
    pm_dir
}
...
fn tools_call_get_returns_front_matter_and_child_count() {
    ...
    let envelope = &resp[0]["result"];
    assert_eq!(envelope["isError"], json!(false));
    let payload: Value = serde_json::from_str(envelope["content"][0]["text"].as_str().unwrap()).unwrap();
...
fn tools_call_add_then_list_includes_new_ticket() {
    ...
    let titles: Vec<&str> = list_payload["tickets"].as_array().unwrap()
        .iter().map(|t| t["title"].as_str().unwrap()).collect();
    assert!(titles.contains(&"Heartbeat"), "new ticket appears in list: {titles:?}");
}
...
fn tools_call_complete_marks_done_and_emits_event() {
    ...
    let get_payload: Value = serde_json::from_str(resp[1]["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    assert_eq!(get_payload["status"], json!("done"));
    let events_payload: Value = serde_json::from_str(resp[2]["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    let verbs: Vec<&str> = events_payload["events"].as_array().unwrap()
        .iter().map(|e| e["verb"].as_str().unwrap()).collect();
    assert!(verbs.contains(&"complete"), "complete event emitted: {verbs:?}");
}
...
fn write_memory_user_scope_returns_is_error() {
    ...
    assert!(text.contains("user"), "stderr-equivalent text mentions user tier");
}
