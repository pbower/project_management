...
fn seed_workspace(pm_dir: &Path) {
    ...
    assert!(
        prd.status.success(),
        "add product: {}",
        String::from_utf8_lossy(&prd.stderr)
    );
    ...
    assert!(
        epc.status.success(),
        "add epic: {}",
        String::from_utf8_lossy(&epc.stderr)
    );
    ...
    assert!(
        tsk.status.success(),
        "add task: {}",
        String::from_utf8_lossy(&tsk.stderr)
    );
}
...
fn artifact_add_lands_in_ARTIFACTS_md() {
    ...
    assert!(
        task_dir.join("artifacts/schema.png").exists(),
    ...
    assert!(
        manifest.contains("file: schema.png"),
        "ARTIFACTS.md missing entry: {manifest}"
    );
}
...
fn move_writes_alias_and_old_address_still_resolves() {
    ...
    let second_epc = pm(
        &dir,
        &["add", "Locking", "--kind", "epic", "--parent", "PRD1"],
    );
    assert!(
        second_epc.status.success(),
        "add second epic: {}",
        String::from_utf8_lossy(&second_epc.stderr)
    );
    ...
    assert!(
        aliases.contains("PRJ1-PRD1-EPC1-TSK1"),
        "alias missing old address: {aliases}"
    );
    assert!(
        aliases.contains("PRJ1-PRD1-EPC2-TSK1"),
...
fn move_emits_event_and_records_move_verb() {
    ...
    let events =
        fs::read_to_string(dir.join("events.log")).expect("events.log should exist after a move");
    assert!(
        events.contains("\"verb\":\"move\""),
